...
| **Live AWS read-only scan collector** | `secureops-scanner::aws::AwsCollector` gated `aws`: S3 public-access-block + encryption, world-open security groups, CloudTrail coverage, root MFA. Rule logic is SDK-free (`aws_rules`) and unit-tested on every CI leg. Select with `SECUREOPS_COLLECTOR=aws`. | Real AWS credentials (read-only role: `s3:ListAllMyBuckets`, `s3:GetBucketPublicAccessBlock`, `s3:GetEncryptionConfiguration`, `ec2:DescribeSecurityGroups`, `cloudtrail:DescribeTrails`, `iam:GetAccountSummary`). |
